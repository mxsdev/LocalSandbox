...
cli
  .command("<input>", "path/url to OpenAPI/Swagger document as json/yaml")
  .option(
    "-o, --output <path>",
  ...
  .option(
    "--error-expr <expr>",
    "Pass an expression to determine if a response status is an error",
  )
  .option(
    "--success-expr <expr>",
  ...
  .option(
    "--export-schemas",
    "When true, will export all `#/components/schemas`",
  )
  .option(
    "--implicit-required",
  ...
  .action(async (input, options) => {
    ...
    const additionalPropertiesDefaultValue = toBoolean(
      options.additionalPropsDefaultValue,
      true,
    )
