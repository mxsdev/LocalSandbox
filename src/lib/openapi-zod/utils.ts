...
export const asComponentSchema = (name: string) =>
...
export const pathParamToVariableName = (name: string) => {
  ...
  return snakeToCamel(preserveUnderscore.replaceAll("-", "_")).replaceAll(
    "#",
    "_",
  )
}
...
export const replaceHyphenatedPath = (path: string) => {
  ...
  matches.forEach((match) => {
    const replacement = pathParamToVariableName(
      match.replaceAll(matcherRegex, ":$1"),
    )
...
export const pathToVariableName = (path: string) =>
  capitalize(kebabToCamel(path).replaceAll("/", "")) // /media-objects/{id} -> MediaObjects{id}
    .replace(pathParamWithBracketsRegex, (group) =>
      capitalize(group.slice(1, -1)),
    ) // {id} -> Id
