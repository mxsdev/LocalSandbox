...
export const getZodClientTemplateContext = (
  ...
  const wrapWithLazyIfNeeded = (schemaName: string) => {
    const [code, ref] = [
      result.zodSchemaByName[schemaName]!,
      result.resolver.resolveSchemaName(schemaName)?.ref,
    ]
  ...
  for (const ref in depsGraphs.deepDependencyGraph) {
    ...
    const schemaName = shouldGenerateType
      ? result.resolver.resolveRef(ref).normalized
      : undefined
    if (shouldGenerateType && schemaName && !data.types[schemaName]) {
      ...
      for (const depRef of depsGraphs.deepDependencyGraph[ref] ?? []) {
        ...
        const isDepCircular =
          depsGraphs.deepDependencyGraph[depRef]?.has(depRef)
  ...
  // TODO
  const schemaOrderedByDependencies = topologicalSort(
    depsGraphs.deepDependencyGraph,
  ).map((ref) => result.resolver.resolveRef(ref).ref)
  ...
  result.endpoints.forEach((endpoint) => {
    ...
    if (groupStrategy !== "none") {
      ...
      const pathItemObject: PathItemObject =
        openApiDoc.paths[endpoint.path] ?? openApiDoc.paths[operationPath]
      if (!pathItemObject) {
      ...
      endpoint.parameters.forEach((param) =>
        addDependencyIfNeeded(param.schema),
      )
      endpoint.errors.forEach((param) => addDependencyIfNeeded(param.schema))
      dependencies.forEach(
        (schemaName) => (group.schemas[schemaName] = data.schemas[schemaName]!),
      )
      ...
      if (groupStrategy.includes("file")) {
        ;[...dependencies].forEach((schemaName) => {
          ...
          depsGraphs.deepDependencyGraph[
            result.resolver.resolveSchemaName(schemaName)?.ref
          ]?.forEach((transitiveRef) => {
            const transitiveSchemaName =
              result.resolver.resolveRef(transitiveRef).normalized
            addDependencyIfNeeded(transitiveSchemaName)
            group.types[transitiveSchemaName] =
              data.types[transitiveSchemaName]!
            group.schemas[transitiveSchemaName] =
              data.schemas[transitiveSchemaName]!
          })
        })
      }
    }
  ...
  if (groupStrategy.includes("file")) {
    ...
    Object.keys(data.endpointsGroups).forEach((groupName) => {
      ...
      group.schemas = sortObjKeysFromArray(
        groupSchemas,
...
type MinimalTemplateContext = Pick<
  TemplateContext,
...
const getOriginalPathWithBrackets = (path: string) =>
  path.replaceAll(originalPathParam, "{$1}")
...
const getPureSchemaNames = (fullSchemaNames: string[]) =>
  fullSchemaNames.map((name) => name.split("/").at(-1)!)
