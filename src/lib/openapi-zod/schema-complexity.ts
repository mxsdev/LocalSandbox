...
export function getSchemaComplexity({
  ...
  if (schema.oneOf) {
    if (schema.oneOf.length === 1) {
      return (
        complexityByComposite("oneOf") +
        getSchemaComplexity({ current, schema: schema.oneOf[0] })
      )
    }
    ...
    return (
      ...
      complexityByComposite("oneOf") +
      getSum(
        schema.oneOf.map((prop) =>
          getSchemaComplexity({ current: 0, schema: prop }),
        ),
      )
    )
  }
  ...
  if (schema.anyOf) {
    if (schema.anyOf.length === 1) {
      return (
        ...
        getSchemaComplexity({ current, schema: schema.anyOf[0] })
      )
    }
  ...
  if (schema.allOf) {
    if (schema.allOf.length === 1) {
      return (
        ...
        getSchemaComplexity({ current, schema: schema.allOf[0] })
      )
    }
  ...
  if (schema.type === "array") {
    if (schema.items) {
      return (
        complexityByComposite("array") +
        getSchemaComplexity({ current, schema: schema.items })
      )
    }

    return (
      complexityByComposite("array") +
      getSchemaComplexity({ current, schema: undefined })
    )
  }
  ...
  if (
    ...
    return (
      complexityByComposite("empty-object") +
      getSchemaComplexity({ current, schema: undefined })
    )
  }
