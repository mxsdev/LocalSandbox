...
export function getZodSchema({
  ...
  const refsPath = code.meta.referencedBy
    .slice(0, -1)
    .map((prev) =>
      ctx ? ctx.resolver.resolveRef(prev.ref!).normalized : prev.ref!,
    )
      ...
      .map((type) => {
        ...
        if ("type" in type.schema) {
          if (Array.isArray(type.schema.type)) {
            ...
            const schemaType = type.schema.type.toLowerCase() as NonNullable<
              typeof schema.type
            >
  ...
  if (schemaType === "array") {
    if (schema.items) {
      return code.assign(
        `z.array(${getZodSchema({
          schema: schema.items,
          ctx,
          meta,
          options,
        }).toString()}${getZodChain({
  ...
  if (
    ...
    const additionalPropsSchema =
      additionalProps === false ? "" : ".passthrough()"
    ...
    const isPartial = options?.withImplicitRequiredProps
      ? false
      : !schema.required?.length
    let properties = "{}"

    if (schema.properties) {
      const propsMap = Object.entries(schema.properties).map(
        ([prop, propSchema]) => {
          const propMetadata = {
            ...meta,
...
export const getZodChain = ({ schema, meta, options }: ZodChainArgs) => {
  ...
  const output = chains
    .concat(
      ...
      options?.withDefaultValues !== false
        ? getZodChainableDefault(schema)
        : [],
    )
    .filter(Boolean)
    .join(".")
