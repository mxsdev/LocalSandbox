...
const getComplexity = (schema: SchemaObject) =>
  getSchemaComplexity({ schema: schema, current: 0 })
...
test("getSchemaComplexity", async () => {
  ...
  expect(
    getComplexity({ type: "array", items: { type: "string" } }),
  ...
  expect(
    getComplexity({ type: "object", additionalProperties: true }),
  ).toMatchInlineSnapshot("1")
  expect(
    getComplexity({ type: "object", additionalProperties: { type: "string" } }),
  ).toMatchInlineSnapshot("2")
  ...
  expect(
    getComplexity({ type: "object", properties: { str: { type: "string" } } }),
  ).toMatchInlineSnapshot("3")
  expect(
    getComplexity({
      type: "object",
      properties: { reference: { $ref: "#/components/schemas/Basic" } },
    }),
  ).toMatchInlineSnapshot("4")
  expect(
    getComplexity({
      ...
      properties: {
        refArray: {
          type: "array",
          items: { $ref: "#/components/schemas/Basic" },
        },
      },
    }),
  ).toMatchInlineSnapshot("5")
  ...
  expect(
    getComplexity({ type: "string", enum: ["aaa", "bbb", "ccc"] }),
  ).toMatchInlineSnapshot("2")
  expect(
    getComplexity({ type: "number", enum: [1, 2, 3, null] }),
  ).toMatchInlineSnapshot("2")
})
