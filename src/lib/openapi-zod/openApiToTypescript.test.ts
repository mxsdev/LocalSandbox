...
const makeSchema = (schema: SchemaObject | OpenAPIV3.SchemaObject) =>
...
test("getSchemaAsTsString", () => {
  ...
  expect(getSchemaAsTsString({ type: "boolean" })).toMatchInlineSnapshot(
    '"boolean"',
  )
  expect(getSchemaAsTsString({ type: "string" })).toMatchInlineSnapshot(
    '"string"',
  )
  expect(getSchemaAsTsString({ type: "number" })).toMatchInlineSnapshot(
    '"number"',
  )
  expect(getSchemaAsTsString({ type: "integer" })).toMatchInlineSnapshot(
    '"number"',
  )
  expect(getSchemaAsTsString({})).toMatchInlineSnapshot('"unknown"')

  expect(
    getSchemaAsTsString({ type: "null" }, { name: "nullType" }),
  ).toMatchInlineSnapshot('"export type nullType = null;"')
  expect(
    getSchemaAsTsString({ type: "boolean" }, { name: "booleanType" }),
  ).toMatchInlineSnapshot('"export type booleanType = boolean;"')
  expect(
    getSchemaAsTsString({ type: "string" }, { name: "stringType" }),
  ).toMatchInlineSnapshot('"export type stringType = string;"')
  expect(
    getSchemaAsTsString({ type: "number" }, { name: "numberType" }),
  ).toMatchInlineSnapshot('"export type numberType = number;"')
  expect(
    getSchemaAsTsString({ type: "integer" }, { name: "integerType" }),
  ).toMatchInlineSnapshot('"export type integerType = number;"')
  expect(
    getSchemaAsTsString({}, { name: "unknownType" }),
  ).toMatchInlineSnapshot('"export type unknownType = unknown;"')

  expect(
    getSchemaAsTsString({ type: "array", items: { type: "string" } }),
  ).toMatchInlineSnapshot('"Array<string>"')
  expect(
    getSchemaAsTsString({ type: "object" }, { name: "EmptyObject" }),
  ).toMatchInlineSnapshot('"export type EmptyObject = {};"')
  expect(
    getSchemaAsTsString(
  ...
  expect(
    ...
    '"export type StringAndNumber = (string & number) | null;"',
  )
  expect(
    getSchemaAsTsString({
      nullable: true,
  ...
  expect(
    ...
    '"export type StringAndNumberMaybeMultiple = (string | number) | Array<string | number>;"',
  )
  ...
  expect(
    getSchemaAsTsString({ type: "number", enum: [1, 2, 3] }),
...
describe("getSchemaAsTsString with context", () => {
  test("with ref", () => {
    ...
    Object.keys(schemas).forEach((key) =>
      ctx.resolver.getSchemaByRef(asComponentSchema(key)),
    )
    expect(
      printTs(
        getTypescriptFromOpenApi({
    ...
    const schemas = {
      Root3: {
        ...
        properties: {
          ...
          arrayOfNested: {
            type: "array",
            items: { $ref: "#/components/schemas/Nested3" },
          },
        },
      },
      Nested3: {
    ...
    const schemas = {
      Root4: {
        ...
        properties: {
          ...
          arrayOfSelf: {
            type: "array",
            items: { $ref: "#/components/schemas/Root4" },
          },
        },
      },
      Nested4: {
...
test("getSchemaAsTsString with readonly", () => {
  ...
  expect(
    getSchemaAsTsString({ type: "null" }, undefined, options),
  ).toMatchInlineSnapshot('"null"')
  expect(
    getSchemaAsTsString({ type: "boolean" }, undefined, options),
  ).toMatchInlineSnapshot('"boolean"')
  expect(
    getSchemaAsTsString({ type: "string" }, undefined, options),
  ).toMatchInlineSnapshot('"string"')
  expect(
    getSchemaAsTsString({ type: "number" }, undefined, options),
  ).toMatchInlineSnapshot('"number"')
  expect(
    getSchemaAsTsString({ type: "integer" }, undefined, options),
  ).toMatchInlineSnapshot('"number"')
  expect(getSchemaAsTsString({}, undefined, options)).toMatchInlineSnapshot(
    '"unknown"',
  )

  expect(
    getSchemaAsTsString({ type: "null" }, { name: "nullType" }, options),
  ).toMatchInlineSnapshot('"export type nullType = null;"')
  expect(
    getSchemaAsTsString({ type: "boolean" }, { name: "booleanType" }, options),
  ).toMatchInlineSnapshot('"export type booleanType = boolean;"')
  expect(
    getSchemaAsTsString({ type: "string" }, { name: "stringType" }, options),
  ).toMatchInlineSnapshot('"export type stringType = string;"')
  expect(
    getSchemaAsTsString({ type: "number" }, { name: "numberType" }, options),
  ).toMatchInlineSnapshot('"export type numberType = number;"')
  expect(
    getSchemaAsTsString({ type: "integer" }, { name: "integerType" }, options),
  ).toMatchInlineSnapshot('"export type integerType = number;"')
  expect(
    getSchemaAsTsString({}, { name: "unknownType" }, options),
  ).toMatchInlineSnapshot('"export type unknownType = unknown;"')
  ...
  ).toMatchInlineSnapshot('"Readonly<Array<string>>"')
  expect(
    getSchemaAsTsString({ type: "object" }, { name: "EmptyObject" }, options),
  ).toMatchInlineSnapshot('"export type EmptyObject = {};"')
  expect(
    getSchemaAsTsString(
      { type: "object", properties: { str: { type: "string" } } },
