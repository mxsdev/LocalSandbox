...
TsConversionArgs): ts.Node | TypeDefinitionObject | string => {
  ...
  const getTs = (): ts.Node | TypeDefinitionObject | string => {
    ...
    if (Array.isArray(schema.type)) {
      ...
      return schema.nullable
        ? t.union([...types, t.reference("null")])
        : t.union(types)
    }
    ...
    if (schema.allOf) {
      ...
      return schema.nullable
        ? t.union([t.intersection(types), t.reference("null")])
        : t.intersection(types)
    }

    const schemaType = schema.type
      ? (schema.type.toLowerCase() as NonNullable<typeof schema.type>)
      : undefined
    if (schemaType && isPrimitiveType(schemaType)) {
      if (schema.enum) {
        if (
          ...
          return schema.nullable
            ? t.union([t.never(), t.reference("null")])
            : t.never()
        }
      ...
      if (schemaType === "string")
        return schema.nullable
          ? t.union([t.string(), t.reference("null")])
          : t.string()
      if (schemaType === "boolean")
        return schema.nullable
          ? t.union([t.boolean(), t.reference("null")])
          : t.boolean()
      if (schemaType === "number" || schemaType === "integer")
        return schema.nullable
          ? t.union([t.number(), t.reference("null")])
          : t.number()
    }
    ...
    if (schemaType === "array") {
      if (schema.items) {
        ...
        if (typeof arrayOfType === "string") {
          if (!ctx)
            throw new Error(
              "Context is required for circular $ref (recursive schemas)",
            )
    ...
    if (
      ...
      const props = Object.fromEntries(
        Object.entries(schema.properties).map(([prop, propSchema]) => {
          ...
          const isRequired = Boolean(
            isPartial ? true : schema.required?.includes(prop),
          )
          return [
            `${wrapWithQuotesIfNeeded(prop)}`,
            isRequired ? propType : t.optional(propType),
          ]
        }),
      )

      const objectType = additionalProperties
        ? t.intersection([props, additionalProperties])
        : props
      ...
      if (!inheritedMeta?.name) {
        throw new Error(
          "Name is required to convert an object schema to a type reference",
        )
      }
  ...
  return canBeWrapped
    ? wrapTypeIfInline({
        ...
        typeDef: tsResult as TypeDefinition,
      })
    : tsResult
}

type SingleType = Exclude<SchemaObject["type"], any[] | undefined>
const isPrimitiveType = (type: SingleType): type is PrimitiveType =>
  primitiveTypeList.includes(type as any)
...
const wrapTypeIfInline = ({
  ...
  if (!isInline) {
    if (!name) {
      throw new Error(
        "Name is required to convert a schema to a type reference",
      )
    }
