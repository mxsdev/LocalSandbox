...
export const getZodiosEndpointDefinitionList = (
  ...
  const isMainResponseStatus = match(options?.isMainResponseStatus)
    .with(
      P.string,
      (option) => (status: number) =>
        sync(option, { status }, { functions: true }),
    )
  ...
  const isMediaTypeAllowed = match(options?.isMediaTypeAllowed)
    .with(
      P.string,
      (option) => (mediaType: string) =>
        sync(option, { mediaType }, { functions: true }),
    )
    .with(
      P.nullish,
      () => (mediaType: string) => mediaType === "application/json",
    )
    .otherwise((fn) => fn)
  ...
  const getZodVarName = (input: CodeMeta, fallbackName?: string) => {
    ...
    if ((result.startsWith("z.") || input.ref === undefined) && fallbackName) {
      ...
      while (
        (isVarNameAlreadyUsed = Boolean(ctx.zodSchemaByName[formatedName]))
      ) {
        if (isVarNameAlreadyUsed) {
          if (
            options?.exportAllNamedSchemas &&
            ctx.schemasByName?.[result]?.includes(formatedName)
          ) {
            return formatedName
      ...
      if (options?.exportAllNamedSchemas && ctx.schemasByName) {
        ctx.schemasByName[result] = (ctx.schemasByName[result] ?? []).concat(
          formatedName,
        )
      }
  ...
  const defaultStatusBehavior =
    options?.defaultStatusBehavior ?? "spec-compliant"
  ...
  for (const path in doc.paths) {
    ...
    for (const method in pathItem) {
      ...
      if (operation.requestBody) {
        ...
        const bodySchema =
          matchingMediaType && requestBody.content?.[matchingMediaType]?.schema
        if (bodySchema) {
      ...
      for (const param of parameters) {
        const paramItem = (
          isReferenceObject(param)
            ? ctx.resolver.getSchemaByRef(param.$ref)
            : param
        ) as ParameterObject
        if (allowedPathInValues.includes(paramItem.in)) {
          ...
          const paramCode = getZodSchema({
            ...
            meta: {
              isRequired:
                paramItem.in === "path" ? true : (paramItem.required ?? false),
            },
            options,
          })
      ...
      for (const statusCode in operation.responses) {
        ...
        const maybeSchema = matchingMediaType
          ? responseItem.content?.[matchingMediaType]?.schema
          : null
      ...
      if (operation.responses?.default) {
        ...
        const maybeSchema =
          matchingMediaType && responseItem.content?.[matchingMediaType]?.schema
        let schemaString = matchingMediaType ? undefined : voidSchema
        let schema: CodeMeta | undefined
      ...
      if (options?.endpointDefinitionRefiner) {
        ...
        endpointDefinition = options.endpointDefinitionRefiner(
          endpointDefinition,
          operation,
        )
      }
...
const getParametersMap = (
  ...
  return Object.fromEntries(
    (parameters ?? []).map(
      (param) =>
        [isReferenceObject(param) ? param.$ref : param.name, param] as const,
    ),
  )
}

const allowedPathInValues = ["query", "header", "path"] as Array<
  ParameterObject["in"]
>
...
> & {
  ...
  parameters: Array<
    Omit<
      Required<ZodiosEndpointDefinition<any>>["parameters"][number],
      "schema"
    > & { schema: string }
  >
  errors: Array<
    Omit<
      Required<ZodiosEndpointDefinition<any>>["errors"][number],
      "schema"
    > & { schema: string }
  >
  responses?: Array<{
...
const isAllowedParamMediaTypes = (
  ...
  | (typeof allowedParamMediaTypes)[number]
