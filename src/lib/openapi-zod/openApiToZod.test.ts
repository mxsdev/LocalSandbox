...
const getSchemaAsZodString = (
  schema: SchemaObject,
  meta?: CodeMetaData | undefined,
) => getZodSchema({ schema: makeSchema(schema), meta }).toString()

test("getSchemaAsZodString", () => {
  expect(getSchemaAsZodString({ type: "null" })).toMatchInlineSnapshot(
    '"z.null()"',
  )
  expect(
    getSchemaAsZodString({ type: "null", enum: ["Dogs", "Cats", "Mice"] }),
  ).toMatchInlineSnapshot('"z.null()"')
  expect(getSchemaAsZodString({ type: "boolean" })).toMatchInlineSnapshot(
    '"z.boolean()"',
  )
  expect(getSchemaAsZodString({ type: "string" })).toMatchInlineSnapshot(
    '"z.string()"',
  )
  expect(getSchemaAsZodString({ type: "number" })).toMatchInlineSnapshot(
    '"z.number()"',
  )
  expect(getSchemaAsZodString({ type: "integer" })).toMatchInlineSnapshot(
    '"z.number()"',
  )
  ...
  expect(
    getSchemaAsZodString({ type: "array", items: { type: "string" } }),
  ...
  expect(
    getSchemaAsZodString({
      type: "object",
      properties: { pa: { type: "number", minimum: 0 } },
    }),
  ).toMatchInlineSnapshot(
  ...
  expect(
    ...
    '"z.object({ pa: z.number().gte(0).lte(100) }).partial().passthrough()"',
  )

  expect(
    getSchemaAsZodString({
      type: "object",
      properties: { ml: { type: "string", minLength: 0 } },
    }),
  ).toMatchInlineSnapshot(
  ...
  expect(
    ...
    '"z.array(z.object({ str: z.string() }).partial().passthrough())"',
  )
  ...
  expect(
    getSchemaAsZodString({ type: "number", enum: [1, 2, 3, null] }),
  ).toMatchInlineSnapshot(
    '"z.union([z.literal(1), z.literal(2), z.literal(3), z.literal(null)])"',
  )
  expect(
    getSchemaAsZodString({ type: "number", enum: [1] }),
  ).toMatchInlineSnapshot('"z.literal(1)"')
  expect(
    getSchemaAsZodString({ type: "string", enum: ["aString"] }),
  ).toMatchInlineSnapshot('"z.literal("aString")"')
})

test("getSchemaWithChainableAsZodString", () => {
  expect(
    getSchemaAsZodString({ type: "string", nullable: true }),
  ).toMatchInlineSnapshot('"z.string()"')
  expect(
    getSchemaAsZodString({ type: "string", nullable: false }),
  ).toMatchInlineSnapshot('"z.string()"')
...
test("CodeMeta with nested refs", () => {
  const schemas = {
    Basic: {
      type: "object",
      properties: { prop: { type: "string" }, second: { type: "number" } },
    },
    WithNested: {
      type: "object",
      properties: {
    ...
    ObjectWithArrayOfRef: {
      ...
      properties: {
        ...
        link: {
          type: "array",
          items: { $ref: "#/components/schemas/WithNested" },
        },
