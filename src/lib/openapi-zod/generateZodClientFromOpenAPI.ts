...
export const generateZodClientFromOpenAPI = async <
  TOptions extends TemplateContext["options"],
>({
  openApiDoc,
  distPath,
  templatePath,
...
> => {
  ...
  if (!templatePath) {
    templatePath = match(groupStrategy)
      ...
      .with("tag", "method", () =>
        path.join(__dirname, "../src/templates/grouped.hbs"),
      )
      .exhaustive()
  }
  ...
  if (groupStrategy.includes("file")) {
    ...
    for (const groupName in data.endpointsGroups) {
      ...
      if (willWriteToFile) {
        ...
        await fs.writeFile(
          path.join(distPath, `${groupName}.ts`),
          prettyGroupOutput,
        )
      }
    }
  ...
  const output = template({
    ...data,
    options: { ...options, apiClientName: options?.apiClientName ?? "api" },
  })
