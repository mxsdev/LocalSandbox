// TODO: move back to configstore package when we support ESM
...
function getConfigDirectory(id, globalConfigPath) {
  ...
  const configDirectory =
    xdgAppPaths({ name: "localsandbox" }).config() ??
    fs.mkdtempSync(fs.realpathSync(os.tmpdir()) + path.sep)
...
export default class Configstore {
  constructor(id, defaults, options = {}) {
    this._path =
      options.configPath ?? getConfigDirectory(id, options.globalConfigPath)
